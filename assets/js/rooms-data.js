...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Update DOM card elements on index.html & rooms.html
  function applyCustomDataToDOM() {
    ...
    Object.keys(window.ROOMS_DETAILS_DATA).forEach(key => {
      const data = window.ROOMS_DETAILS_DATA[key];
      const roomContainer = document.getElementById(key);
      if (roomContainer) {
        // 1. If it's a .room-card (on index.html)
        if (roomContainer.classList.contains('room-card')) {
          const thumb = roomContainer.querySelector('.room-thumb');
          if (thumb && data.cover) thumb.src = data.cover;

          const priceSpan = roomContainer.querySelector('.room-price-tag span:nth-child(2)');
          if (priceSpan && data.price) {
            priceSpan.textContent = data.price.includes('₫') || data.price.includes('VND') || data.price.includes('$') ? data.price : `${data.price}₫`;
          }

          const titleLink = roomContainer.querySelector('.room-title a');
          if (titleLink) titleLink.textContent = (currentLang === 'vi' ? data.nameVi : data.nameEn) || data.name || titleLink.textContent;
        ...
        // 2. If it's a .dining-banner (on rooms.html)
        if (roomContainer.classList.contains('dining-banner')) {
          const img = roomContainer.querySelector('.dining-img');
          if (img && data.cover) img.src = data.cover;

          const title = roomContainer.querySelector('.section-title');
          if (title) title.textContent = (currentLang === 'vi' ? data.nameVi : data.nameEn) || data.name || title.textContent;

          const desc = roomContainer.querySelector('.dining-info > p');
          if (desc) desc.textContent = (currentLang === 'vi' ? data.descVi : data.descEn) || desc.textContent;

          const metaValues = roomContainer.querySelectorAll('.dining-meta-item .value');
  ...
  window.openRoomDetailModal = function(roomKey) {
    ...
    const currentLang = localStorage.getItem('sala_lang') || 'en';
    const roomTitle = currentLang === 'vi' ? room.nameVi : room.nameEn;
    const roomDesc = currentLang === 'vi' ? room.descVi : room.descEn;
      ...
      <div class="room-modal-body">
        <div class="room-modal-title-row">
          ...
          <div class="room-modal-price-badge">
            ...
            <div class="room-modal-price-sub">/ ${currentLang === 'vi' ? 'đêm (bao gồm buffet sáng)' : 'night (includes breakfast)'}</div>
          </div>
        </div>
        ...
        <div class="room-specs-grid">
          <div class="room-spec-box">
            ...
            <div class="room-spec-label">${currentLang === 'vi' ? 'Diện tích' : 'Room Size'}</div>
            <div class="room-spec-val">${room.size}</div>
          </div>
          <div class="room-spec-box">
            <i class="fa-solid fa-user-group"></i>
            <div class="room-spec-label">${currentLang === 'vi' ? 'Sức chứa' : 'Occupancy'}</div>
            <div class="room-spec-val">${room.guests}</div>
          </div>
          <div class="room-spec-box">
          ...
          <div class="room-spec-box">
            ...
            <div class="room-spec-label">${currentLang === 'vi' ? 'Tầm nhìn' : 'Balcony & View'}</div>
            <div class="room-spec-val">${room.view}</div>
          </div>
        </div>
        ...
        <div style="font-weight: 700; color: var(--dark-bg); font-size: 1.05rem; margin-bottom: 10px; border-bottom: 1px solid #eee; padding-bottom: 6px;">
          <i class="fa-solid fa-star" style="color: var(--primary-gold);"></i> ${currentLang === 'vi' ? 'Tiện Nghi Phòng Cao Cấp (In-Room Amenities)' : 'In-Room Luxury Amenities'}
        </div>
        ...
        <div class="room-amenities-pills">
          ...
          <div class="room-amenity-pill"><i class="fa-solid fa-mug-hot"></i> ${currentLang === 'vi' ? 'Ấm đun nước, Trà & Cà phê miễn phí' : 'Free Tea, Coffee & Mineral Water'}</div>
          <div class="room-amenity-pill"><i class="fa-solid fa-shirt"></i> ${currentLang === 'vi' ? 'Áo choàng tắm & Dép đi trong phòng' : 'Soft Bathrobes & Slippers'}</div>
          <div class="room-amenity-pill"><i class="fa-solid fa-wind"></i> ${currentLang === 'vi' ? 'Máy sấy tóc & Đồ vệ sinh cá nhân' : 'Hairdryer & Deluxe Toiletries'}</div>
          <div class="room-amenity-pill"><i class="fa-solid fa-bell-concierge"></i> ${currentLang === 'vi' ? 'Dịch vụ dọn phòng hàng ngày 24/7' : 'Daily Housekeeping Service'}</div>
        </div>
  ...
  // Delegate click for all View Details buttons
  document.addEventListener('click', (e) => {
    const target = e.target.closest('[data-i18n="btn_view_details"], [data-open-room-detail]');
    if (target) {
      const roomCard = target.closest('.room-card, .dining-banner');
      let roomKey = target.getAttribute('data-open-room-detail');

      if (!roomKey && roomCard) {
        roomKey = roomCard.id;
        if (!roomKey) {
          const bookingBtn = roomCard.querySelector('[data-open-booking]');
          if (bookingBtn) roomKey = bookingBtn.getAttribute('data-open-booking');
        }
      }
